...
3. `svd`:特異値分解を利用してマージします。rankを設定することで、自由にマージ先のrankを変えることができます。ただし時間がかかるほか、精度も少し落ちます。`device`でGPUで計算するかCPUで計算するか選べます。thresholdを1.0未満にすると、rankを自動的に決定するモードに変わります。

## リサイズについて
lora2を省略して、`mode`を`svd`にすると、リサイズモードになります。
